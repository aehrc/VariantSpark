...
class DelegatingLineIterator(val it:Iterator[String]) extends AbstractIterator[String] with LineIterator {
...
class VCFSource(val lines:RDD[String], val headerLines:Int=500) {
    ...
    lazy val headerAndVersion = {
      ...
      val header:VCFHeader = codec.readActualHeader(new DelegatingLineIterator(lines.take(headerLines).toIterator)).asInstanceOf[VCFHeader]
...
object VCFSource {
  ...
  def apply(lines:RDD[String], headerLines:Int=500):VCFSource = new VCFSource(lines, headerLines)
  def apply(sc:SparkContext, fileName:String, headerLines:Int):VCFSource = apply(sc.textFile(fileName), headerLines)
