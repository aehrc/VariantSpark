...
trait RandomForestArgs {
  ...
  @Option(name="-rmd", required=false, usage="Maximum depth a tree should go after which it should stop splitting.",
      aliases=Array("--rf-max-depth"))
  val rfMaxDepth:Int = Int.MaxValue
 
  @Option(name="-rmns", required=false, usage="Minimum node size after which tree should stop splitting.",
