...
class GenerateLabelsCmd extends ArgsApp with SparkApp with Echoable with Logging with TestArgs {
  ...
  @Option(
    name = "-gs",
    required = false,
    usage = "Generator effect noise stddev (def=0.0)",
  ...
  @Option(
    name = "-gm",
    required = false,
    usage = "Generator effect noise man (def=0.0)",
  ...
  @Option(
    name = "-on",
    required = false,
    usage = "Output noise variables (false)",
  ...
  def run(): Unit = {
    ...
    val generator = EffectLabelGenerator(featureSource)(
      zeroLevel = actualZeroLevel,
      effects = effects,
      noiseEffectSigma = noiseSigma,
      noiseEffectMean = noiseMean,
      noiseVarFraction = noiseVarFraction,
      seed = randomSeed)
    ...
    CSVUtils.withFile(new File(featuresFile)) { writer =>
      ...
      val outputColumns =
        List(featureSource.sampleNames, labels.toList) ::: (if (featureContinuousColumn != null)
                                                              List(
                                                                generator.continouusResponse.data.toList)
