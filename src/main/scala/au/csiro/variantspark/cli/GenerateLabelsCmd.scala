...
class GenerateLabelsCmd extends ArgsApp with SparkApp with Echoable with Logging with TestArgs {
  ...
  def run(): Unit = {
    ...
    echo(
        s"Generating a dichotomous response, zeroLevel: ${actualZeroLevel}, noiseSigma: ${noiseSigma}")
    ...
    echo(
        s"Continous response mean: ${generator.continuousStats.mean} , total variance: ${generator.continuousStats.variance}")
    if (isEcho) {
      effects.toStream
        .sortBy(_._1)
        .map {
