...
class GenerateLabelsCmd extends ArgsApp with SparkApp with Echoable with Logging with TestArgs {
  ...
  // generator options
  @Option(name = "-gs", required = false, usage = "Generator effect noise stddev (def=0.0)",
    aliases = Array("--gen-noise-sigma"))
  val noiseSigma: Double = 0.0

  @Option(name = "-gm", required = false, usage = "Generator effect noise man (def=0.0)",
    aliases = Array("--gen-noise-mean"))
  val noiseMean: Double = 0.0

  @Option(name = "-gvf", required = false,
    usage = "Generator fraction of noise variables (def=0.0)",
    aliases = Array("--gen-noise-fraction"))
  ...
  @Option(name = "-on", required = false, usage = "Output noise variables (false)",
    aliases = Array("--output-noise-vars"))
  val outputNoiseVars: Boolean = false

  // output options
  @Option(name = "-onf", required = false,
    usage = "Path to file where to save noise variables (stdout)",
  ...
  override def testArgs: Array[String] =
    Array("-if", "target/getds.parquet", "-sp", "4", "-ff", "target/features.csv", "-fc", "resp",
      "-sr", "133", "-v", "-on", "-fcc", "resp_cont", "-fiv", "-ge", "v_0:1.0", "-ge", "v_1:1.0",
  ...
  override def run(): Unit = {
    ...
    val generator = EffectLabelGenerator(featureSource)(zeroLevel = actualZeroLevel,
      effects = effects, noiseEffectSigma = noiseSigma, noiseEffectMean = noiseMean,
    ...
    echo(
        s"Continous response mean: ${generator.continuousStats.mean}, "
          + s" total variance: ${generator.continuousStats.variance}")
    if (isEcho) {
      effects.toStream
        .sortBy(_._1)
        .map {
          case (v, e) =>
            ...
            (s"${v} -> e=${e},  v=${varVariance},"
              + s" R2=${varVariance / generator.continuousStats.variance}")
        }
        .foreach(println)
    }
    if (outputNoiseVars) {
    ...
    CSVUtils.withFile(new File(featuresFile)) { writer =>
      ...
      val continuousVarData = if (featureContinuousColumn != null) {
        List(generator.continouusResponse.data.toList)
      } else { Nil }
      val outputColumns =
        List(featureSource.sampleNames,
          labels.toList) ::: continuousVarData ::: effectVarData.toList
          .map(_._2.toList)
