...
object StdFeature {
  def from[V](label: String, variableType: VariableType, v: V)(
      implicit db: DataBuilder[V]): Feature = {
    StdFeature(label, variableType, db.from(v))
  }
  def from[V](label: String, v: V)(implicit db: DataBuilder[V]): Feature =
    from(label, db.defaultVariableType, v)
  def from[V](label: String, variableType: VariableType, v: List[String])(
      implicit db: DataBuilder[V]): Feature = StdFeature(label, variableType, db.from(v))
}
