...
class WideDecisionTreeClassificationIntegratedTest extends SparkTest {
  ...
  def checkCNAE_9_Dataset(maxDepth: Int, dataType: VariableType = BoundedOrdinalVariable(5)) {
    ...
    // check predictions
    val expected = TestCsvUtils.readColumnToIntArray(
        "src/test/data/CNAE-9_R_predictions_classification.csv", s"maxdepth_${maxDepth}")
    ...
    val expectedImportances = TestCsvUtils.readColumnToDoubleArray(
        "src/test/data/CNAE-9_R_importance_classification.csv", s"maxdepth_${maxDepth}")
