...
class ModelConverterTest {
  ...
  val rfModel =
    RandomForestModel(List(RandomForestMember(DecisionTreeModel(SplitNode(1, classCounts0, 4, 0.6,
              0L, ThresholdSplitCriteria(1.0), 0.3, LeafNode(0, classCounts0_0, 1, 0.0),
  ...
  def testConvertsSimpleModelCorrectlyWithEmptyMapping() {
    ...
    val expectedRepresntation = Forest(None,
      Seq(Tree(Split(1, classCounts0, 4, 0.6, null, 0L, false, ThresholdSplitCriteria(1.0), 0.3, Leaf(0, classCounts0_0, 1, 0.0), Leaf(3, classCounts0_1, 1, 0.1)), Some(OOBInfo(oobIndexes, oobPredictions))), Tree(Leaf(1, classCounts1, 2, 0.5), None)),
      Some(oobErrors))
  ...
  def testConvertsSimpleModelCorrectlyWithExistingMapping() {
    val expectedRepresntation = Forest(None,
      Seq(Tree(Split(1, classCounts0, 4, 0.6, "VAR_0", 0L, false, ThresholdSplitCriteria(1.0), 0.3, Leaf(0, classCounts0_0, 1, 0.0), Leaf(3, classCounts0_1, 1, 0.1)), Some(OOBInfo(oobIndexes, oobPredictions))), Tree(Leaf(1, classCounts1, 2, 0.5), None)),
      Some(oobErrors))
