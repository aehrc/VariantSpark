...
class ReproducibilityTest extends SparkTest {
  ...
  def testReproducibleResults() {    
    ...
    val impAnalysis1 = features.importanceAnalysis(label, nTrees = 2, seed = Some(13L), mtryFraction = None, batchSize = 20)
    val topVariables1 = impAnalysis1.importantVariables(20)
    topVariables1.foreach(println)
    val impAnalysis2 = features.importanceAnalysis(label, nTrees = 2, seed = Some(13L), mtryFraction = None, batchSize = 20)
