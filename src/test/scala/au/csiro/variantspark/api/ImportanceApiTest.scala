...
class ImportanceApiTest extends SparkTest {
  ...
  def testImportanceAnalysisLegacyApi() {
    ...
    val importanceAnalysis = ImportanceAnalysis(fs, ls, nTrees = 200, batchSize=50, seed = Some(17L))
  ...
  def testImportanceAnalysisNewApi() {    
    ...
    val impAnalysis = features.importanceAnalysis(label, nTrees = 200, batchSize=50, seed = Some(17L))
